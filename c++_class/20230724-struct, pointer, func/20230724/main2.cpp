...
// 비어있는 구조체라도 변수를 선언한 것이니 1byte 공간 차지
// tmi(무언가 공부할때 항상 검증하기 -> 면접 시에도 구글링 했다고X )
struct TestStr
{
...
int main()
{
	/*
		...
		포인터 변수의 크기를 x86이면 4바이트 x64는 8바이트이다.
		...
		그러므로 포인터 변수를 사용할 때는 반드시 0 혹은 NULL 혹은 nullptr로 초기화하고 사용하는 것이 좋다.
		0, NULL은 C언어에서 주로 사용하던 방식이고, C++에서는 nullptr로 초기화 한다.
		nullptr로 초기화 하면 0으로 초기화된다.
	*/

	int Number = 100;

	// 아래는 int 포인터 타입 변수인 NumberAddr을 선언하고 Number의 메모리 주소를 NumberAddr 변수의 값으로 대입해주겠다.
	int* NumberAddr = &Number;

	/*
	포인터 변수가 다른 변수의 메모리 주소를 저장하고 있다면 해당 메모리 주소를 참조하고 있다 라고 표현한다.
	
	포인터 변수에 다른 변수의 메모리 주소가 저장되어 있을 경우 역참조를 통해서 해당 메모리 주소에 접근하여 거기에 있는 값을 얻어오거나 변경할 수 있다.
	역참조 때문에 타입에 맞는 주소를 저장하게 되어 있는 것이다.
	int 포인터라면 정수이기 때문에 역참조로 정수를 처리하고 float 포인터라면 실수이기때문에 역참조로 실수를 처리하는 방식이다.
	*포인터명 을 하면 포인터가 가지고 있는 메모리 주소에 역참조를 수행한다.
	
	*/
	// 아래와 같이 역참조로 접근하여 500을 넣으면 NumberAddr은 Number변수의 메모리 주소를 저장하고 있으므로 Number에 500을 넣어주는 것과 같다.
	*NumberAddr = 500;

	std::cout << "Number = " << Number << std::endl;
	std::cout << "Number Address = " << &Number << std::endl;
	std::cout << "NumberAddr = " << NumberAddr << std::endl;
	std::cout << "NumberAddr Address = " << &NumberAddr << std::endl; // Number변수 주소를 저장하고 있는 변수의 주소
	...
	/*
		...
		이 주소는 int타입의 주소이므로 int의 포인터 타입 변수에 메모리 주소를 저장할 수 있다.

		배열의 인덱스에 접근할 때 배열이름[인덱스]로 접근한다.
		배열시작주소[인덱스] 로 접근하는 것이다.
		만약 포인터변수가 배열의 메모리 주소를 가지고 있을 경우 같은 방법으로 인덱스 접근이 가능하다.

		int Array[100] = { 1, 2, 3, 4, 5 };
		int* ArrayAddr = Array;

		위와 같이 선언되었다면 ArrayAddr는 배열의 시작주소를 가지고 있으므로 이 포인터 변수를 이용해서 배열의 각 요소에 Array를 이용해서 접근하는 것과 같이 접근할 수 있다.
	*/
	int Array[100] = {1, 2, 3, 4, 5};
	int* ArrayAddr = Array;
	...
	*/
	...
	// void 포인터에 Number 변수 메모리 주소 할당
	void* voidAddr = &Number;

	// void 포인터를 float타입으로 형변환 후 값을 할당
	*((float*)voidAddr) = 5.32f;
	...
	/*
		...
		1을 더하면 단순히 메모리주소가 1증가하는 것이 아닌 타입의 크기만큼 증가한다.
		int 포인터라면 int 크기인 4만큼 증가하는 것이다.
	...
	*/
	...
	std::cout << "*(ArrayAddr + 0) = " << *(ArrayAddr +0) << std::endl;
	std::cout << "*(ArrayAddr + 0) = " << *(ArrayAddr +1) << std::endl;
	std::cout << "*(ArrayAddr + 0) = " << *(ArrayAddr +2) << std::endl;

	/*
	포인터에서 const를 붙이는 방법은 3가지가 있다.
	변수처럼 앞, 뒤, 둘다 3가지가 있다.
	const int* Addr;
	int* const Addr;
	...
	*/
	...
	// const가 앞에 붙을 경우 참조하는 대상 메모리주소는 변경이 가능하지만 참조하는 대상의 값은 변경할 수 없다.
	const int* Addr1 = &Number; // const int* 타입으로 Number 변수 메모리 주소 할당 
	Addr1 = &Number2;  // 메모리주소 Number2 변수 메모리 주소로 변경
	//*Addr1 = 500; // 변경 불가능

	// const가 타입의 뒤(오른쪽)에 붙을 경우 참조하는 대상 메모리주소를 변경할 수 없다.
	int* const Addr2 = &Number;
	// Addr2 = &Number2; // 메모리 주소 변경 불가능
	*Addr2 = 500;  // 값 변경 가능
	...
	*/
	...
	/*
	이중포인터 : 포인터 변수에 메모리 주소를 담아놓기 위한 변수이다.
	int**
	*/
	...
	*/
	...
	// 만약 그 주소를 가지고 있지 않을 경우 메모리를 해제할 수 없다. (주소를 알아야 삭제 가능)
