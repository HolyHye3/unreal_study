...
순수가상함수: 가상함수를 자식클래스에서 무조건 재정의하게 만들어주느 기능이다.
순수가상함수는 일반 가상함수와 다르게 자식클래스에 재정의가 없을 경우 에러가 발생한다.

순수가상함수를 1개 이상 가지고 있는 클래스를 추상클래스라고 한다.
이때 자식클래스에서 부모클래스의 순수가상함수를 재정의 하지 않을 경우 해당 자식클래스도 추상클래스가 된다.

추상클래스는 포인터타입 변수는 선언이 가능하지만 실제 객체는 생성할 수 없는 클래스이다.
