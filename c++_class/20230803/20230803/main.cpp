...
형태
template <typename 원하는타입이름>

typename 대신에 class 키워드를 이용해도 된다.
template <class 원하는타입이름>

타입 이름은 여러개 지정이 가능하다.
template <typename 원하는이름1, typename 원하는이름2> 이런 방식으로 여러개를 사용할 수
있다.
...
// T라는 변수타입을 만들어주었다. 이 T는 이 함수를 호출할 때마다 원하는 타입을 지정하여
// 호출할 수 있다.
template <typename T>
...
public:
	// 클래스 전체에 대한 템플릿이 있다고 하더라도 개별적인 멤버함수별로 따로
	// 템플릿을 사용할 수도 있다.
...
int main()
{
	...
	*/
	...
	IntArray.TestOutput<ETest>();
	FloatArray.TestOutput<FTest>();
