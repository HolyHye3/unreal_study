...
void CObject::Update(float DeltaTime)
{
	/*
		...
		200, 0 이 나오게 될 것이다.
		여기에 인자로 들어온 델타타임을 곱해주게 되면 초당 200만큼의 거리를 이동하는 식이 완성된다.

		속도: 200
		델타타임: 1/60

		200 x (1/60)
		델타타임이 1/60초라면 200을 1초동안 곱했을대 60번 곱하게 되므로 총 누적되는 합은 200/60x60이 되어 200이 나오게 된다.
		*/
...
{
	Rectangle(hDC, (int)mPos.x, (int)mPos.y, (int)(mPos.x + mSize.x), (int)(mPos.y + mSize.y));
